...
const buscarCarreraPorId = async (req, res) => {
  try {
    ...
    if (!idCarrera) {
      res
        .status(404)
        .json({ estado: "FALLO", msj: "Falta el id de la carrera" })
    }
...
const buscarTodasCarreras = async (req, res) => {
  try {
...
const eliminarCarrera = async (req, res) => {
  try {
    ...
    if (!idCarrera) {
      res.status(404).json({
        estado: "FALLO",
        msj: "Especifica el ID de la carrera que quieres eliminar",
      })
    } else {
      try {
        ...
        res.status(500).json({
          estado: "FALLO",
          msj: "Error al intentar eliminar la carrera",
        })
      }
    }
  } catch (exec) {
...
const crearCarrera = async (req, res) => {
  ...
  if (!nombre || !modadlida) {
    ...
    try {
      ...
      res
        .status(201)
        .json({ estado: "OK", msj: "Carrera creada", dato: carreraNueva })
    } catch (exec) {
      throw exec
    }
  }
...
const actualizarCarrera = async (req, res) => {
  try {
    ...
    if (!idCarrera) {
      res.status(404).json({
        estado: "FALLO",
        msj: "Falta el id de la carrera que quieres actualizar",
      })
      ...
      try {
        ...
        res.json({
          estado: "OK",
          msj: "Carrera actualizada exitosamente",
          dato: carreraActualizada,
        })
      } catch (error) {
        console.error(error)
        res.status(500).json({
          estado: "FALLO",
          msj: "Error al intentar actualizar la carrera",
        })
      }
    }
  } catch (exec) {
...
const obtenerEstudiantesCarrera = async (req, res) => {
  try {
    ...
    const estudiantes = await carreraBD.obtenerEstudiantesCarrera(idCarrera)
