...
const eliminar = async (req, res) => {
  try {
    ...
    if (!idEstudiante) {
      res.status(404).json({
        estado: "FALLO",
        msj: "Especifica el ID del estudiante que queres eliminar",
      })
    } else {
      try {
...
const crear = async (req, res) => {
  ...
  if (!dni || !nombre || !apellido || !nacionalidad || !correoElectronico) {
    ...
    try {
      ...
      res
        .status(201)
        .json({ estado: "ok", msj: "Estudiante creado", dato: estudianteNuevo })
    } catch (exec) {
      throw exec
    }
  }
...
const actualizar = async (req, res) => {
  try {
    ...
    if (!idEstudiante) {
      res.status(404).json({
        estado: "FALLO",
        msj: "Falta el id del estudiante que quieres actualizar",
      })
    } else if (
      !dni ||
      !nombre ||
      ...
      res.status(400).json({
        estado: "FALLO",
        msj: "Falta algun dato necesario para actualizar",
      })
    } else {
      ...
      try {
        ...
        res.json({
          estado: "OK",
          msj: "Estudiante actualizado exitosamente",
          dato: estudianteActualizado,
        })
      } catch (error) {
        console.error(error)
        res.status(500).json({
          estado: "FALLO",
          msj: "Error al intentar actualizar el estudiante",
        })
      }
    }
  } catch (exec) {
...
const obtenerCarrerasEstudianteController = async (req, res) => {
  try {
    ...
    const carreras = await estudianteBD.obtenerCarrerasEstudiante(idEstudiante)
...
const obtenerMateriasEstudianteController = async (req, res) => {
  try {
    ...
    const materias = await estudianteBD.obtenerMateriasEstudiante(idEstudiante)
